...
function createAppServer(options){
  ...
  createWebsocketServer({server: server, app: app}, function(sockets){
    ...
    app.get('/car/home', function (req, res){
      for( var c = 0; c < sockets.clients.length; c++ ) {
        sockets.clients[c].send( 'car/home' );
      }
      return res.send({signalSent: 'car/home'});
    });
    app.get('/car/forward/:speed', function (req, res){
      for(var c = 0; c < sockets.clients.length; c++) {
        sockets.clients[c].send( 'car/forward/'+ req.params.speed);
      }
      return res.send({signalSent: 'car/forward/'+ req.params.speed});
    });
    app.get('/car/backward/:speed', function (req, res){
      for(var c = 0; c < sockets.clients.length; c++) {
        sockets.clients[c].send( 'car/backward/'+ req.params.speed);
      }
      return res.send({signalSent: 'car/backward/'+ req.params.speed});
    });
    app.get('/car/turn/:angle', function (req, res){
      for(var c = 0; c < sockets.clients.length; c++) {
        sockets.clients[c].send( 'car/turn/'+ req.params.angle);
      }
      return res.send({signalSent: 'car/turn/'+ req.params.angle});
    });
    ...
    app.get('/system/cpu_temp', function (req, res){
      for( var c = 0; c < sockets.clients.length; c++ ) {
        sockets.clients[c].send( 'system/cpu_temp' );
      }
      return res.send({signalSent: 'system/cpu_temp'});
    });
  });
}
